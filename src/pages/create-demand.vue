<template>
  <v-dialog v-model="modalVisible">
    ...
    >
      <v-toolbar dark>
        <v-toolbar-title>Подача заявки</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-btn
            icon
            @click="modalCanHide"
      ...
      <v-card-text class="editor-main-card-body">
        <v-form ref="form" v-model="valid">
          ...
          <date-time-input
              label="Когда"
              v-model.trim="form.after"
              required
              :error-messages="errors('after')"
          ...
          <v-textarea
              ...
              counter="255"
              :rules="[validation.maximumLength(255)]"
